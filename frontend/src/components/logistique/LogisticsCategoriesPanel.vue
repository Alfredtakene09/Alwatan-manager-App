...
const items = ref<LogisticsCategoryRecord[]>([])
...
const tableRows = computed(() =>
  items.value.map((item) => ({
    ...
    itemsLabel: `${item.itemsCount} article(s)`,
...
async function loadItems() {
  ...
  message.value = ''
  try {
    const { data } = await api.get<LogisticsCategoryRecord[]>('/logistique/categories')
    items.value = data
  } catch {
...
async function saveItem() {
  ...
  try {
    if (isEditing.value && editingId.value) {
      await api.put(`/logistique/categories/${editingId.value}`, payload)
      message.value = 'Catégorie modifiée.'
    } else {
      await api.post('/logistique/categories', payload)
...
async function toggleItem(id: string) {
  ...
  if (!item) return
  try {
    await api.put(`/logistique/categories/${id}`, { active: !item.active })
...
<template>
  <PageTableSection embedded>
    ...
    <p v-if="!loading && !items.length" class="empty">Aucune catégorie — créez-en une pour classer vos articles.</p>
    <UiDataTable
      v-else
      fill
      table-key="logistics-categories"
      compact
      :data="tableRows"
      :columns="columns"
      :loading="loading"
  ...
  <UiFormModal
    v-if="modalOpen"
    title-id="logistics-category-modal-title"
  ...
  >
    ...
    <section class="form-panel">
      <UiInput v-model="formName" label="Nom" placeholder="Ex. Consommables médicaux" required />

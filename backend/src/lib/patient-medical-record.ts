...
/** Direction / gestionnaire : tous les patients suivis par un médecin, avec résumé clinique. */
export async function getDirectionDossierPatients() {
  const visits = await prisma.visit.findMany({
    where: { consultation: { isNot: null } },
    include: {
      patient: {
        select: {
          id: true,
  ...
  for (const visit of visits) {
    ...
    const entry = buildMedicalHistoryEntry(
      ...
      { labValidatedOnly: false, canViewAllClinicalDetails: true },
    );

    const validatedAt = consultation?.completedAt ?? consultation?.updatedAt ?? visit.updatedAt;
    ...
    if (new Date(validatedAt).getTime() > new Date(existing.lastVisitAt).getTime()) {
      existing.lastVisitAt = validatedAt.toISOString();
      if (doctorName) existing.doctorName = doctorName;
    }
  }

...
export function validateInterventionPercents(surgeonPercent: number, anesthesiologistPercent: number) {
  ...
  if (surgeonPercent + anesthesiologistPercent > 100) {
    return "La somme des pourcentages chirurgien et assistant chirurgie ne peut pas dépasser 100 %.";
  }
  if (anesthesiologistPercent > 0 && clinicPercentFromSplits(surgeonPercent, anesthesiologistPercent) < 0) {
    return "Répartition invalide : la clinique ne peut pas être négative.";
  }
  return null;
}

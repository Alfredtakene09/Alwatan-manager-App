...
export async function countUserRelatedData(userId: string): Promise<number> {
  const counts = await Promise.all([
    ...
    prisma.hospitalization.count({ where: { attendingDoctorId: userId } }),
    prisma.invoice.count({ where: { issuedById: userId } }),
    prisma.invoicePayment.count({ where: { recordedById: userId } }),
    ...
    prisma.employeePayroll.count({ where: { paidById: userId } }),
    prisma.salaryAdvance.count({ where: { recordedById: userId } }),
    prisma.stockMovement.count({ where: { userId } }),
    prisma.logisticsStockMovement.count({ where: { userId } }),
    prisma.logisticsRequest.count({ where: { requestedById: userId } }),
    prisma.logisticsRequest.count({ where: { handledById: userId } }),
  ]);
